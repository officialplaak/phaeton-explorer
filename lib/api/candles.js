	...
	this.getStatistics = function (e, error, success) {
		...
		if (!config.marketWatcher.enabled) {
			const statistics = {
				...
				phaetonVolume: Number(0).toFixed(8),
				numTrades: 0,
			};
		...
		(err, results) => {
			...
			let statistics = {
				...
				phaetonVolume: _.reduce(results[0], (memo, t) =>
					(memo + parseFloat(t.phaetonVolume)), 0.0).toFixed(8),
			...
			statistics = {
				...
				phaetonVolume: statistics.phaetonVolume,
