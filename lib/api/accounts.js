...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getVotesCall = (account, limit, offset, cb) => {
			request.get({
				url: `${app.get('PHA address')}/votes?address=${coreUtils.parseAddress(account.address)}&limit=${limit}&offset=${offset}&sort=username:asc`,
				json: true,
			}, (err, response, body) => {
		...
		this.getVotersCall = (account, limit, offset, cb) => {
			request.get({
				url: `${app.get('PHA address')}/voters?address=${coreUtils.parseAddress(account.address)}&limit=${limit}&offset=${offset}&sort=username:asc`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getAccount = (params, cb) => {
			...
			return request.get({
				url: `${app.get('PHA address')}/accounts?${query}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getMultisignaturesGroups = (account, cb) => {
			request.get({
				url: `${app.get('PHA address')}/accounts/${coreUtils.parseAddress(account.address)}/multisignature_groups`,
				json: true,
			}, (err, response, body) => {
		...
		this.getMultisignaturesMemberships = (account, cb) => {
			request.get({
				url: `${app.get('PHA address')}/accounts/${account.address}/multisignature_memberships`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getIncomingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('PHA address')}/transactions?recipientId=${coreUtils.parseAddress(account.address)}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getOutgoingTxsCnt = (account, cb) => {
			...
			return request.get({
				url: `${app.get('PHA address')}/transactions?senderId=${coreUtils.parseAddress(account.address)}&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getForged = (account, cb) => {
			...
			return request.get({
				url: `${app.get('PHA address')}/delegates/${coreUtils.parseAddress(account.address)}/forging_statistics`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getTopAccounts = (query, error, success) => {
		...
		return request.get({
			url: `${app.get('PHA address')}/accounts/?sort=balance:desc&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
