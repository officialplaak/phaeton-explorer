...
module.exports = function (app) {
	...
	const Transaction = function () {
		this.getDelegate = function (forger, cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates?publicKey=${forger.publicKey}`,
				json: true,
			}, (err, response, body) => {
	...
	this.getTransaction = function (transactionId, error, success, url) {
		...
		const getInitialData = (waterCb) => {
			request.get({
				url: app.get('PHA address') + url + transactionId,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
	...
	this.getUnconfirmedTransactions = function (transactions, error, success) {
		async.waterfall([
			cb => request.get({
				url: `${app.get('PHA address')}/node/transactions/unconfirmed`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	this.getLastTransactions = function (query, error, success) {
		request.get({
			url: `${app.get('PHA address')}/transactions?sort=timestamp:desc&limit=20`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	this.getTransactionsByAddress = function (query, error, success) {
		...
		const queryCallsList = queryList.map(directionQuery => (callback) => {
			request.get({
				url: `${app.get('PHA address')}/transactions?${directionQuery}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
	...
	this.getTransactionsByBlock = function (query, error, success) {
		...
		return async.waterfall([
			function (cb) {
				request.get({
					url: `${app.get('PHA address')}/transactions?blockId=${query.blockId}&sort=timestamp:desc&offset=${param(query.offset, 0)}&limit=${param(query.limit, 100)}`,
					json: true,
				}, (err, response, body) => {
					if (err || response.statusCode !== 200) {
						return error({ success: false, error: (err || 'Response was unsuccessful') });
