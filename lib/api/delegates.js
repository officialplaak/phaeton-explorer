...
module.exports = function (app) {
	...
	function Delegates() {
		this.getAllDelegates = function (limit, offset, cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates/?sort=rank:asc&limit=${limit}&offset=${offset}`,
				json: true,
			}, (err, response, body) => {
		...
		this.getDelegatesFromTimestamp = function (fromTimestamp, limit, offset, cb) {
			request.get({
				url: `${app.get('PHA address')}/transactions/?sort=timestamp:desc&type=2&limit=${limit}&offset=${offset}&fromTimestamp=${fromTimestamp || 0}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	function Active() {
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates/?sort=rank:asc&limit=101`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getForged = function (delegate, cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates/${delegate.address}/forging_statistics`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			url: `${app.get('PHA address')}/delegates/?sort=rank:asc&limit=${delegates.limit}&offset=${delegates.actualOffset}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
	...
	function Registrations() {
		this.getTransactions = function (cb) {
			request.get({
				url: `${app.get('PHA address')}/transactions/?sort=timestamp:desc&limit=5&type=2`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = function (tx, cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates?publicKey=${tx.senderPublicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	function Votes() {
		this.getVotes = (cb) => {
			request.get({
				url: `${app.get('PHA address')}/transactions/?sort=timestamp:desc&limit=5&type=3`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getNextForgers = function (temp, error, success) {
		request.get({
			url: `${app.get('PHA address')}/delegates/forgers?limit=101&offset=0`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
	...
	function LastBlock() {
		this.getBlock = function (cb) {
			request.get({
				url: `${app.get('PHA address')}/blocks?&sort=height:desc&limit=1`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getDelegate = function (block, cb) {
			request.get({
				url: `${app.get('PHA address')}/delegates?publicKey=${block.generatorPublicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getLastBlocks = function (params, error, success) {
		...
		return request.get({
			url: `${app.get('PHA address')}/blocks?sort=height:desc&generatorPublicKey=${params.publicKey}&limit=${params.limit}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: err });
	...
	this.getSearch = function (params, error, success) {
		...
		return request.get({
			url: `${app.get('PHA address')}/delegates?search=${params}`,
			json: true,
		}, (err, response, body) => {
