...
module.exports = function (app) {
	this.getBlockHeight = function (error, success) {
		request.get({
			url: `${app.get('PHA address')}/node/status`,
			json: true,
		}, (err, response, body) => {
			if (err) {
	...
	this.getBlockByHeight = function (height, error, success) {
		request.get({
			url: `${app.get('PHA address')}/blocks?height=${height}`,
			json: true,
		}, (err, response, body) => {
		...
		this.getBlockHeight(
			...
			(data) => {
				if (data.success === true) {
					return request.get({
						url: `${app.get('PHA address')}/blocks?sort=height:desc&limit=20&offset=${blocks.offset(n)}`,
						json: true,
					}, (err, response, body) => {
	...
	function Block() {
		...
		this.getBlock = function (blockId, height, cb) {
			request.get({
				url: `${app.get('PHA address')}/blocks?blockId=${blockId}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
